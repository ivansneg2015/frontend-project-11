...
const updateInputStyle = (error) => {
  elements.inputElement.classList.toggle('error-border', !!error);
};

const validateRss = (url, watchedState) => {
  const links = watchedState.feeds.map((feed) => feed.rss);
        ...
        .then((response) => {
          ...
          elements.postsContainer.addEventListener('click', (event) => {
            if (event.target.tagName === 'A' || event.target.tagName === 'BUTTON') {
              ...
              const targetPost = watchedState.posts.find((post) => post.id === targetID);
